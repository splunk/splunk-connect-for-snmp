...
for i in "${files[@]}"
do
  if [ -f $i ]; then f=$i; else f=https://github.com/splunk/splunk-connect-for-snmp/blob/$BRANCH/$i; fi
  $KCMD -n sc4snmp create -f $f 2>/dev/null
done
...
do
  if [ -f $i ]; then f=$i; else f=https://github.com/splunk/splunk-connect-for-snmp/blob/$BRANCH/$i; fi
  $KCMD -n sc4snmp apply -f $f 2>/dev/null
done
