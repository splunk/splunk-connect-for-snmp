...
class TestTasks(TestCase):
    ...
    @patch('time.time')
    def test_trap_retry_translation(self, m_time, m_load_mib, m_is_mib_known, m_process_data, m_resolved,  *mongo_args):
        from splunk_connect_for_snmp.snmp.tasks import trap
        m_time.return_value = 1640692955.365186
    ...
    @patch('splunk_connect_for_snmp.snmp.manager.Poller.load_mibs')
    @patch('time.time')
    def test_trap_retry_translation_failed(self, m_time, m_load_mib, m_is_mib_known, m_process_data, m_resolved, *mongo_args):
        from splunk_connect_for_snmp.snmp.tasks import trap
        m_time.return_value = 1640692955.365186
