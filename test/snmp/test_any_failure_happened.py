...
class TestAnyFailureHappened(TestCase):
    ...
    def test__any_failure_happened_default(self):
        ...
        self.assertEqual("An error of SNMP isWalk=True for a host 127.0.0.1 occurred: Empty SNMP response message",
                         str(ctx.exception))
    ...
    def test__any_failure_happened_ignore_true(self):
        error_indication = EmptyResponse("Empty SNMP response message")
        self.assertEqual(_any_failure_happened(error_indication, 0, 0, [], "127.0.0.1", True), False)
