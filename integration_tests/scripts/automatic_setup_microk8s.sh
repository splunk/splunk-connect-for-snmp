...
sudo microk8s kubectl create -n sc4snmp secret generic autodiscovery-v3-md5-aes --dry-run=client -o yaml --from-literal=userName=autodiscovery-md5 --from-literal=authKey=AuthPass2 --from-literal=privKey=PrivPass2 --from-literal=authProtocol=MD5 --from-literal=privProtocol=AES --from-literal=securityEngineId=8000000903000A3900000102 | sudo microk8s kubectl apply -f -
