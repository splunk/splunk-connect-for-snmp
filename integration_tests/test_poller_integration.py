...
def test_poller_integration_event(setup_splunk):
    ...
    search_string = 'search index="netops" sourcetype="sc4snmp:event" earliest=-1m'
...
def test_poller_integration_metric(setup_splunk):
    ...
    search_string = "| mcatalog values(metric_name) where index=netmetrics AND metric_name=sc4snmp.* earliest=-1m"
