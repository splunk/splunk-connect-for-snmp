...
COMMON_PORTS = [22, 80, 8080]

class Discovery(Task):
    def __init__(self):
        self.snmp_engine = SnmpEngine()

    def tcp_ping(self, ip, is_ipv6, timeout=5):
    ...
    def discover_active_hosts(self, subnet, is_ipv6, network, max_threads=10):
        """Scan subnet for active host using nmap"""
        """
        Scan all hosts in the given subnet (IPv4 or IPv6).
        Returns a list of IPs that responded to ping.
        """
        active = []

        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            futures = {executor.submit(self.tcp_ping, ip, is_ipv6): ip for ip in network.hosts()}
            for future in as_completed(futures):
                try:
                    ...
                    logger.error(
                        f"Pinging device {futures[future]} generated an exception : {e}"
                    )
    ...
    def get_host_list(self, subnet, skip_active_check, is_ipv6):
        ...
        try:
            ...
            if skip_active_check:
                ...
                hosts = self.discover_active_hosts(subnet, is_ipv6, network)
                return hosts
        except Exception as e:
