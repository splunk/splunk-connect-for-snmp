...
def get_mongo_client() -> pymongo.MongoClient:
    ...
    forked - i.e. from within a task body or a post-fork handler such as
    ``before_start`` - never from a ``Task.__init__``, which runs once in the
    master before any child is forked (a MongoClient created there and inherited
    across ``fork()`` is unsafe per pymongo).
    """
