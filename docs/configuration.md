...
**traps-server-config.yaml**. Non-authorized traps/informs will be
dropped.
...
Inventory.csv acts as a lookup table where the poller application will
...
params, which can be set up in config.yaml.
...
which can be set up in config.yaml.
...
1.  In **scheduler-inventory.yaml**, add the profile string(e.g. router)
...
2.  In **scheduler-config.yaml**, add the desired query information
...
#### 4. Configure poller to return query with additional fields present
...
##### Existing VarBinds
This section works only for IF-MIB oid family.
...
```
From now on in every query for IF-MIB two new fields "interface_index" and "interface_desc" are visible:
```yaml
oid-type1="ObjectIdentity" value1-type="Integer" 1.3.6.1.2.1.2.2.1.7.2="1" value1="1" IF-MIB::ifAdminStatus.2="up" interface_index="2" interface_desc="eth0"
```
The value of newly added properties is calculated according to current query index, for IF-MIB::ifAdminStatus.**2** we're interested in IF-MIB::ifIndex.**2** and IF-MIB::ifDescr.**2**.
```yaml
...
-   Besides changing the inventory contents under section
    **inventory.csv**, you may need to set up security passphrases for
    the SNMPv3 under section **config.yaml > usernames**.
...
2.  Specify other security params under section **config.yaml >
    usernames**.
