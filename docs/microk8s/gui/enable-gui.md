...
| `UI.auth.secureCookies`      | When `true`, the session cookie is issued with the `Secure` flag and is only transmitted over HTTPS. Keep `true` in any environment reachable outside `localhost`. Set to `false` only for local development over plain HTTP — the cookie will not be accepted by modern browsers over HTTPS otherwise.                                                                                                           | `true`  | no                                |
| `UI.allowedOrigins`          | Comma-separated list of origins allowed to call the UI backend (e.g. `https://snmp-ui.example.com,https://snmp-ui.internal`). If empty and `auth.enabled=true`, the backend defaults to `http://localhost:8080`. Set explicitly in production to the exact URL(s) a browser will use to reach the UI — otherwise login requests will be rejected by CORS. | `""`    | recommended in production         |
